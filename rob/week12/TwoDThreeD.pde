...
class TwoDThreeD {
  ...
  float distance2D(PVector v1, PVector v2){
    float x = v1.x - v2.x;
    float y = v1.y - v2.y;
    return sqrt(x*x + y*y);
  }
  ...
  List<PVector> sortCorners(List<PVector> quad) {
    ...
    for (int i=0; i<4; ++i) {
      float distTemp = distance2D(quad.get(i), new PVector(0, 0));
      if (distTemp < dist) {
        closestC = i;
        dist = distTemp;
      }
    }
    Collections.rotate(quad, closestC);
    return quad;
  }
  ...
  double[][] solveExtrinsicMatrix(List<PVector> points2D) {
    ...
    /************************************** DONE? **************************************/
    for (int i=0; i<4; i++) {
      float[] temp = {points2D.get(i).x, points2D.get(i).y, 1};
      for(int j=0; j<3; ++j){
        projectedCorners[i][j] = temp[0]*invK[0][j] + temp[1]*invK[1][j] + temp[2]*invK[2][j];
      }
    }
