...
class QuadGraph {
  ...
  void filter(ArrayList<PVector> lines, int width, int height) {
    for (int i = 0; i < cycles.size(); i++) {
      ...
      float areaPreset = width*height/10;
      if ( !isConvex(c1, c2, c3, c4) 
           || !nonFlatQuad(c1, c2, c3, c4) 
           || !validArea(c1, c2, c3, c4, areaPreset*9, areaPreset) ) {
        cycles.remove(i);
        --i;
      }
    }
  ...
  float area(PVector c1, PVector c2, PVector c3, PVector c4){
    ...
    return Math.abs(0.5f * (i1 + i2 + i3 + i4)) / 2;
  }
  ...
  List<PVector> sortCorners(List<PVector> quad) {
    ...
    Collections.sort(quad, new GraphCWComparator(center));
